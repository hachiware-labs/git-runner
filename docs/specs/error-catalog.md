...
| JetStream delivery is combined with `--no-require-worker` | `submit` | `2` | no | conflicting options |
...
| no matching worker accepts core submit dispatch | `submit` | `4` | no | routing tag, explicit core mode, and default JetStream alternative |
