...
重要: MVP は NATS core publish/subscribe を使っており、durable queue ではありません。default では、`submit` は publish 前に一致する ready worker の応答を確認します。worker が応答しない場合、pending job を残さずに失敗します。
...
readiness guard を bypass する場合は、`--no-require-worker` を渡します。
